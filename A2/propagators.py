...
def prop_GAC(csp, newVar=None):
    ...
    if newVar is not None:
        ...
        for val in newVar.cur_domain():
            if newVar.get_assigned_value() != val:
                pruned_variable_values.append((newVar, val))
                newVar.prune_val(val)
    else:
        q = csp.get_all_cons()
        ...
        for var in constraint.get_scope():
            for val in var.cur_domain():
                if not constraint.has_support(var, val):
                    pruned_variable_values.append((var, val))
                    var.prune_value(val)
                    if len(var.cur_domain()) == 0:
                        return False, pruned_variable_values
                    for new_constraint in csp.get_cons_with_var(var):
                        if new_constraint not in q:
                            q.append(new_constraint)
    return True, pruned_variable_values
...
def ord_mrv(csp):
    ...
    for var in csp.get_all_vars():
        if not var.is_assigned() and len(var.cur_domain()) < mrv:
            mrv = len(var.cur_domain())
            mrv_var = var
    return mrv_var
