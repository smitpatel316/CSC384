...
Asia = BN(
    "Asia",
    [VisitAsia, Smoking, Tuberculosis, Cancer, Bronchitis, TBorCA, Dyspnea, Xray],
...
if __name__ == "__main__":
    ...
    if probs1[0] == 0.5 and probs1[1] == 0.5 and probs2[0] == 0.0 and probs2[1] == 1.0:
        ...
        print("failed.")
    print(
        "P(g|s) = {} P(-g|s) = {} P(g|-s) = {} P(-g|-s) = {}".format(
    ...
    if probs1[0] == 0.5 and probs1[1] == 0.5 and probs2[0] == 0.5 and probs2[1] == 0.5:
        ...
        print("failed.")
    print(
        "P(g|s,w) = {} P(-g|s,w) = {} P(g|s,-w) = {} P(-g|s,-w) = {}".format(
            probs1[0], probs1[1], probs2[0], probs2[1]
        )
    )
    ...
    print(
        "P(g|-s,w) = {} P(-g|-s,w) = {} P(g|-s,-w) = {} P(-g|-s,-w) = {}".format(
            probs3[0], probs3[1], probs4[0], probs4[1]
        )
    )
    ...
    probs2 = VE(Q3, G, [W])
    if (
        abs(probs1[0] - 0.15265998457979954) < 0.0001
        and abs(probs1[1] - 0.8473400154202004) < 0.0001
        and abs(probs2[0] - 0.01336753983256819) < 0.0001
        and abs(probs2[1] - 0.9866324601674318) < 0.0001
    ):
        print("passed.")
    else:
        print("failed.")
    print(
        "P(g|w) = {} P(-g|w) = {} P(g|-w) = {} P(-g|-w) = {}".format(
            probs1[0], probs1[1], probs2[0], probs2[1]
        )
    )
    ...
    print("Test 7 ....", end="")
    probs = VE(Q3, G, [])
    if abs(probs[0] - 0.04950000000000001) < 0.0001 and abs(probs[1] - 0.9505) < 0.0001:
        print("passed.")
    else:
        print("failed.")
    ...
    if all(tests):
        ...
        print("failed.")
    print(
        "P(e,b) = {} P(-e,b) = {} P(e,-b) = {} P(-e,-b) = {}".format(
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    values.append(value)
            tests.append(
                value == FE.get_value([e_val]) * FS.get_value([s_val, e_val, b_val])
            )
    if all(tests):
        print("passed.")
    else:
        print("failed.")
    print(
        "P(e,s,b) = {} P(-e,s,b) = {} P(e,-s,-b) = {} P(-e,s,-b) = {}".format(
            values[0], values[1], values[2], values[3]
        )
    )
    ...
    tests = (
        abs(values[0] - 1.7) < 0.001,
        abs(values[1] - 0.2) < 0.001,
        abs(values[2] - 0.3) < 0.001,
    ...
    if norm_sum == 1:
        ...
        print("failed.")
    print(
        "{} when normalized to {} sum to {}".format(
            [i for i in range(5)], normalized_nums, norm_sum
        )
    )
    ...
    print(
        "{} when normalized to {} sum to {}".format(
            [i for i in range(0, -5, -1)], normalized_nums, norm_sum
        )
    )
    ...
    print(
        "{} when normalized to {} sum to {}".format(
            [i for i in range(4, -5, -1)], normalized_nums, norm_sum
        )
    )
